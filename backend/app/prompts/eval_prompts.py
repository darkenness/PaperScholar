"""Evaluation prompts for diagram and plot quality assessment.
Ported from PaperBanana's prompts/diagram_eval_prompts.py and prompts/plot_eval_prompts.py.

Each dimension uses a referenced comparison approach:
  - Human-drawn (ground truth) vs Model-generated image
  - Outputs a JSON with comparison_reasoning and winner
"""
...
def get_eval_prompt(task_type: str, dimension: str) -> str:
    """Get the evaluation system prompt for a given task type and dimension."""
    key = f"{task_type.upper()}_REFERENCED_COMPARISON_{dimension.upper()}_SYSTEM_PROMPT"
