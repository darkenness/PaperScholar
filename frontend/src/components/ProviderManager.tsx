...
export default function ProviderManager({system=false}:{system?:boolean}){
  ...
  const discover=async()=>{if(!editing)return;setBusy(b=>({...b,models:true}));try{const data=await api.models(editing.id);setModels(data.models);toast.info(data.message);}catch(e:any){setFormError(e.message||'读取失败，可手动填写模型 ID');}finally{setBusy(b=>({...b,models:false}));}};
  ...
  return <section className="space-y-4">
    ...
    {modal&&<Dialog title={modal==='create'?'建立连接并添加模型':modal==='clone'?'为此连接添加模型':'编辑模型配置'} onClose={()=>{if(!saving){setModal(null);setForm(empty());}}}>
      <form onSubmit={save} className="space-y-4">
        ...
        {editing&&<button type="button" className="text-sm text-primary-600" disabled={busy.models} onClick={discover}>{busy.models?'读取中…':'从已保存连接读取模型列表'}</button>}
