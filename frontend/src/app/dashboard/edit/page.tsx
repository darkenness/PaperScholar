...
export default function EditPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[var(--text-primary)]">图表编辑</h1>
        <p className="text-[var(--text-muted)] text-sm mt-1">上传图片进行矢量化编辑，或直接生成 SVG 矢量图</p>
      </div>

      {/* Tab switcher */}
      <div className="flex gap-2">
        <button onClick={() => setSvgMode(false)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${!svgMode ? 'bg-primary-600 text-white' : 'bg-[var(--badge-bg)] text-[var(--text-muted)]'}`}>
          图片矢量化
        </button>
      ...
      {!svgMode ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div className="tech-panel p-5">
              <h3 className="text-sm font-bold text-[var(--text-primary)] mb-3">上传图表</h3>
              <label className="block border-2 border-dashed border-[var(--border-main)] rounded-xl p-8 text-center cursor-pointer hover:border-primary-500/50 transition-colors">
            ...
            <div className="tech-panel p-5 space-y-3">
              <h3 className="text-sm font-bold text-[var(--text-primary)]">SAM3 分割设置</h3>
              <div>
                <label className="text-[11px] text-[var(--text-muted)] mb-1 block">SAM3 后端</label>
              ...
              <div>
                <label className="text-[11px] text-[var(--text-muted)] mb-1 block">SAM3 API Key</label>
              ...
              <div>
                <label className="text-[11px] text-[var(--text-muted)] mb-1 block">检测提示词 (逗号分隔)</label>
          ...
          <div className="space-y-4">
            {result ? (
              <>
                <div className="tech-panel p-5">
                  <h3 className="text-sm font-bold text-[var(--text-primary)] mb-3">检测结果</h3>
                  <p className="text-sm text-[var(--text-muted)] mb-2">检测到 <span className="text-primary-400 font-bold">{result.icon_count}</span> 个图标区域</p>
                  {result.icons?.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                      {result.icons.map((ic: any) => (
                        <span key={ic.label} className="px-2 py-1 bg-[var(--badge-bg)] rounded text-xs text-[var(--text-secondary)] font-mono">{ic.label} ({ic.width}x{ic.height})</span>
                      ))}
                    </div>
                  )}
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="space-y-4">
            <div className="tech-panel p-5">
              <h3 className="text-sm font-bold text-[var(--text-primary)] mb-3">图表描述</h3>
            ...
            <div className="tech-panel p-5">
              <h3 className="text-sm font-bold text-[var(--text-primary)] mb-3">方法内容 (可选)</h3>
