    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">共 {total} 位用户</p>
        </div>
            ...
            <tbody className="divide-y divide-white/5 text-sm">
              {users.map((u: any) => (
                <tr key={u.id} className="hover:bg-white/5 transition-colors">
                  <td className="px-5 py-3 font-mono text-gray-500 text-xs">{u.id}</td>
                  ...
                  <td className="px-5 py-3 text-xs text-gray-500 font-mono">{new Date(u.created_at).toLocaleDateString()}</td>
                  <td className="px-5 py-3 text-right space-x-2">
                    <button onClick={() => handleToggleRole(u.id, u.role)} className="text-xs text-gray-400 hover:text-white border border-gray-600 hover:border-gray-400 px-2 py-1 rounded transition-all">
